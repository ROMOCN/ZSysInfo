...
{
    _labelName->setStyleSheet(QString("color:rgb(%1,%2,%3);").arg(color.red()).arg(color.green()).arg(color.blue()));
    _labelValue->setStyleSheet(QString("color:rgb(%1,%2,%3);").arg(color.red()).arg(color.green()).arg(color.blue()));
}
...
void My_ShowWidget::editInit(QLabel *edit)
{
...
void My_ShowWidget::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if(mousePressed)
    {
        ...
        {
//            int distance = (event->pos() - point).manhattanLength();
